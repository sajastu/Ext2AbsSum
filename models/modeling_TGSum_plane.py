...
def normal_shift_right(input_ids):
    shifted_input_ids = input_ids.new_zeros(input_ids.shape)
    shifted_input_ids[:, 1:] = input_ids[:, :-1].clone()
...
class TGSumModel(LEDModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            # TODO
            # get sentence and section rerpresentations...
            sent_repr = self.get_repr_from_index(encoder_outputs[0], index=torch.index(input_ids==0))
            sect_repr = self.get_repr_from_index(encoder_outputs[0], index=torch.index(input_ids==self.BOSECT_ID))

            sent_scores = self.Sigmoid(
                self.sent_scorer(sent_repr)
            )
            sect_scores = self.Softmax(
                self.sect_scorer(sect_repr)
            )
            LIMIT = 4096  # tokens
            ...
            """
            # ... should move sent_scores to shape [1, SECT_NUM, N_SENTS]
            # ...
            sent_scores_sectionized = []
            section_sent_len = [] # section_sent_len be in shape [1, N_SECTS, N_SENTS]

            # get cumulative count of sent_scores_sectionized
            sent_scores_sectionized_cum = []

            # *** get top n scores (from last dim) with indices, where n is "sample_sect_dict"...
            # ** should somehow convert n to number of tokens using "section_sent_len"
            # indices are used to fetch from encoder_outputs...

            top_encoder_outputs = [] # in shaped [1, 4096, 1024]

            for idx, sample_tokens in enumerate(sample_sect_dist):
                sample_tokens = int(sample_tokens)
                # sort padded_sect_sent_scores[:, idx] with indices
                sorted, indices = sorted(padded_sect_sent_scores[:, idx])
                sampled = 0
                counter = 0
                sampled_lst = []
                while sampled < sample_tokens:
                    sampled += section_sent_len[0, idx, indices[counter]]
                    counter+=1
                    sampled_lst.append(indices[counter])

                if len(sampled_lst) > 0:

                    for sent_index in sampled_lst:
                        top_encoder_outputs.append(
                            encoder_outputs[0, ]
                        )
            ...
            ####################
            # should compute m losses for sent_scorer and sect_scorer; one time processing

            # now create encoder outputs...

            # sample_sect_dist should be of shape [1, 8], where the second dim is the sampling len for a specific summary...

            # split input_ids to get section sents.. (should be done)
            # assume that input_ids is like [1, 8, 489], where 1 is bsz, 8 is #sections and 489 is padded tokens...


            # should check... sample the first n ids of a tensor based on an index tensor (sample_sect_dist)
            encoder_outputs_tmp = []
            for i in range(sample_sect_dist.size(1)):
                sampling_len = sample_sect_dist[0, i].item()
                encoder_outputs_tmp.append(
                    int(sampling_len)
                )
            ...
            # pass to another Transformers for prediction...
